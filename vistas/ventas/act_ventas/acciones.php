...
switch ($_GET['sw']) {
       ...
       break;
        ...
        case 5:
            ...
            break;
        case 6:
            $request = mysqli_query($con,"SELECT count(*), concat(b.nombre_cont,' ',b.apellido_cont) from actividad a, sis_contacto b  where a.id_contacto=b.id_contacto and a.estado = 'Planificada' and a.StartTime = '".date("Y-m-d H:i")."' and a.notificacion=0  and a.reg_user='".$_SESSION['k_username']."'   ");
            $r =  mysqli_fetch_row($request);
            echo $r[1];
            if($r[0]>0){
        ...
        case 8:
            ...
            $hora = $_GET['hora'];
            $fsh=$fecha.' '.$hora;
            $query = mysqli_query($con,"select * from actividad where Id=$id ");
            $c = mysqli_fetch_array($query);
            mysqli_query($con,"insert into actividad (`Subject`,`Description`,`StartTime`,`estado`,`prioridad`,`aviso`,`user`,`id_contacto`,`EndTime`,`reg_user`,`id_tercero`) "
            . "values ('".$c['Subject']."','','".$fsh."','Planificada','".$c['prioridad']."','".$c['aviso']."','".$c['user']."','".$c['id_contacto']."','".$fsh."','".$_SESSION['k_username']."','".$c['id_tercero']."')");
