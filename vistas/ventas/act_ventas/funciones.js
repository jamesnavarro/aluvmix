...
function mostrar_actividades(page){
     ...
     var cli= $("#clienteb").val();
        $.ajax({
            ...
            url: '../vistas/ventas/act_ventas/lista.php',
      ...
      if(est===''){
       ...
       if(client===''){
        ...
        $.ajax({
            ...
            url: '../vistas/ventas/act_ventas/acciones.php', 
            success: function(resultado){
                $("#id_a").val(resultado); 
...
function reprogramar(id){
         window.open("../vistas/ventas/act_ventas/reprogramar.php?id="+id, "Rep", "width=500 , height=200");
}
...
function add_reprogramar(id){
    ...
    var hra = $("#hra").val();
    if(fec_ini==''){
        alert("Debes de seleccionar la fecha");
        $("#fec_ini").focus();
    }
    if(hra==''){
        alert("Debes de seleccionar la hora");
        $("#hra").focus();
    }
      var c = confirm("Se reprogramara esta actividad?");
     if(c){
             $.ajax({
            type: 'GET',
            ...
            url: 'acciones.php', 
            success: function(resultado){
                alert("Se ha programo la actividad con exito "+resultado);
...
function borrar(id){
     var page = $("#page").val();
     var c = confirm("Se cambiara el estado a completada ?");
     if(c){
         $.ajax({
            type: 'GET',
            data: 'id='+id+'&sw=3',  //
