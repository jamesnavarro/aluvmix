...
if(isset($_SESSION['k_username'])){ 
   $fechs=  $_GET['nuefecha'];
   $page= $_GET['page'];
            $request = mysqli_query($con,"SELECT count(*) FROM  seguimientos  where id_s = ".$_GET['rad']." and fecha_registros like '%".$fechs."%' ");
            if($request){
  ...
  <table style="width: 95%" class="table table-hover">
    ...
    $query = mysqli_query($con,"SELECT *FROM  seguimientos  where id_s = ".$_GET['rad']." and fecha_registros like '%".$fechs."%' order by id_seguimiento desc ".$limit);
