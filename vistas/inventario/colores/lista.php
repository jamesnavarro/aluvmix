...
if(isset($_SESSION['k_username'])){
    ...
    $res=$_GET['res'];
   $page= $_GET['page'];
            $request = mysqli_query($con,"SELECT count(*) FROM colores where  color like '%".$des."%' and nom_resumido like '%".$res."%' ");
...
$request_ac = mysqli_query($con,"SELECT * FROM colores where  color like '%".$des."%' and nom_resumido like '%".$res."%'  " .$limit );
 $total2=0;
	while($fila=mysqli_fetch_array($request_ac))
	{  
 if ($fila['estado_cco']== '1') {
