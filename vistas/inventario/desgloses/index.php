<?php
  ...
  $querymart = mysql_query("select color,comprar from desgloses_material where id_cot='".$_GET["cot"]."' group by id_cot ") or die(mysql_error());
    $row3 = mysql_fetch_array($querymart);
    if($row3[0]=='02'){
        $pin = 'PINTURA';
    }else if($row3[0]=='01'){
        $pin = 'ANONIZADO';
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Desgloses Pendientes </title>
