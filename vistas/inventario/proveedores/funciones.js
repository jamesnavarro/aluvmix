...
function mostrar_line2(page){
        ...
        $.ajax({
                ...
                url: '../vistas/inventario/proveedores/lista2.php',
            success: function(d){
...
function buscar_cod(){
       ...
       $.ajax({
                type: 'GET',
                data: 'cod='+cod+'&sw=1',
                url: '../vistas/inventario/proveedores/acciones.php',
         success: function(t) {
             var t = eval(t);
              $("#codxa").val(cod);
              $("#refxa").val(t[1]);
...
function con_fom_cod(page){
    ...
    $("#mostrar_tabla2").html('<tr><td colspan="2">Cargando<img src="../images/load.gif"></td>');
      $.ajax({
          type:'GET',
          url:'http://172.16.0.30:8989/api/MAECXP/'+cod+'/'+page+'/10',
          dataType: 'json',
          success: function(da){
...
<input type="hidden" id="use'+dev.CLI_CEDULA+'" value="'+dev.CLI_USUARIO+'">\n\
              ...
              '<td>'+dev.CLI_NOMBRE+'</td>'+
              '<td>'+dev.CLI_TELEFO+'</td>'+
              '<td>'+dev.CLI_EMAIL+'</td>'+
              '<td style="text-align:center" id="ch'+dev.CLI_CEDULA+'"><input type="checkbox" name="item" id="'+dev.CLI_CEDULA+'" value="'+dev.CLI_CEDULA+'"></td>\n\
</tr>';
  return row;
