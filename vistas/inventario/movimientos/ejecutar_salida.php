...
if(mysqli_num_rows($xql)==0){
	...
	if($act){
		...
		$sql=mysqli_query($con,"INSERT INTO `mov_inventario`(`fecha_pro`, `codigo_tm`, `num_documento`, `doc_referencia`, `bod_codigo`, `bod_destino`, `cen_codigo`, `usuario`, `save_mov`, `id_orden`, `tipo_movimiento`, `codigo_ter`, `nombre_tercero`, `total`, `diferencia`, `cod_empresa`, `id_puesto`) VALUES ('".$_POST['FecReg']."','".$_POST['tipdoc']."','".$_POST['factura']."','".$rax['max(ult_cons)']."','".$_POST['loc']."','".$_POST['loc2']."','".$_POST['cc']."','".$_POST['por']."','0','".$_POST['compra']."','".$_POST['descarga']."','".$_POST['nombrepro']."','".$_POST['nter']."','".$_POST['totalx']."','".$_POST['diferencia']."','".$_SESSION['empresa']."','".$_POST['idpues']."')");
		if($sql){
			$max=0;
			$save=0;
			$fecha='';
