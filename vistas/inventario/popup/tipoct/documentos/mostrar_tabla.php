...
$request=mysqli_query($con,"SELECT count(*) FROM tipo_cuentas where concat(codigo,' ',descripcion) like '%".$_GET['nombre']."%'");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM tipo_cuentas where concat(codigo,' ',descripcion) like '%".$_GET['nombre']."%'".$limit);
			$item = 0;
			if(mysqli_num_rows($sql)>0){
				while($mostrar = mysqli_fetch_array($sql)){
					$item = $item+1;
                                        $codigo= "'".$mostrar['codigo']."'".",'".trim($mostrar['descripcion'])."'";
					echo '<tr>
<td>'.$mostrar['codigo'].'</td>
<td><a href="#PasarVariable" onclick="pasar('.$codigo.')">'.$mostrar['descripcion'].'</a></td>'; }
			}else{
