...
$request=mysqli_query($con,"SELECT count(*) FROM bodegas where concat(bod_nombre,' ',bod_codigo) like '%".$_GET['nombre']."%'");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM bodegas where concat(bod_nombre,' ',bod_codigo) like '%".$_GET['nombre']."%'".$limit);
			$item = 0;
			if(mysqli_num_rows($sql)>0){
				while($mostrar = mysqli_fetch_array($sql)){
					$item = $item+1;
                                        $codigo= "'".$mostrar['bod_codigo']."'".",'".trim($mostrar['bod_nombre'])."','".trim($mostrar['sede'])."'";
					echo '<tr>
<td>'.$mostrar['bod_codigo'].'</td>
<td><a href="#PasarVariable" onclick="pasar('.$codigo.')">'.$mostrar['bod_nombre'].'</a></td>'; }
			}else{
