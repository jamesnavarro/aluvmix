...
$request=mysqli_query($con,"SELECT count(*) FROM grupos where concat(gru_nombre,' ',gru_codigo) like '%".$_GET['nombre']."%'");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM grupos where concat(gru_nombre,' ',gru_codigo) like '%".$_GET['nombre']."%'".$limit);
			$item = 0;
			if(mysqli_num_rows($sql)>0){
				while($mostrar = mysqli_fetch_array($sql)){
					$item = $item+1;
                                        $codigo= "'".$mostrar['gru_codigo']."'";
					echo '<tr>
<td>'.$mostrar['gru_codigo'].'</td>
<td><a href="#PasarVariable" onclick="pasar('.$codigo.')">'.$mostrar['gru_nombre'].'</a></td>'; }
			}else{
