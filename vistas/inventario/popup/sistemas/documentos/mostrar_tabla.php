...
$request=mysqli_query($con,"SELECT count(*) FROM sistemas where concat(nombre_sistema) like '%".$_GET['nombre']."%'");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM sistemas where concat(id_sistema,' ',nombre_sistema) like '%".$_GET['nombre']."%'".$limit);
			$item = 0;
			if(mysqli_num_rows($sql)>0){
				while($mostrar = mysqli_fetch_array($sql)){
					$item = $item+1;
                                        $codigo= "'".trim($mostrar['nombre_sistema'])."'";
					echo '<tr>
<td>'.$mostrar['id_sistema'].'</td>
<td><a href="#PasarVariable" onclick="pasar('.$codigo.')">'.$mostrar['nombre_sistema'].'</a></td>'; }
			}else{
