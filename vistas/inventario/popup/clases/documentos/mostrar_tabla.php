...
$request=mysqli_query($con,"SELECT count(*) FROM clases where concat(cla_nombre,' ',cla_codigo) like '%".$_GET['nombre']."%'");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM clases where concat(cla_nombre,' ',cla_codigo) like '%".$_GET['nombre']."%'".$limit);
			$item = 0;
			if(mysqli_num_rows($sql)>0){
				while($mostrar = mysqli_fetch_array($sql)){
					$item = $item+1;
                                        $codigo= "'".$mostrar['cla_codigo']."'";
					echo '<tr>
<td>'.$mostrar['cla_codigo'].'</td>
<td><a href="#PasarVariable" onclick="pasar('.$codigo.')">'.$mostrar['cla_nombre'].'</a></td>'; }
			}else{
