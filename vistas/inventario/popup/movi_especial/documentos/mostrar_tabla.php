...
$request=mysqli_query($con,"SELECT count(*) FROM tipos_movimientos where concat(movimiento,' ',codigo_tm) like '%".$_GET['nombre']."%'");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM tipos_movimientos where concat(movimiento,' ',codigo_tm) like '%".$_GET['nombre']."%'".$limit);
			$item = 0;
			if(mysqli_num_rows($sql)>0){
				while($mostrar = mysqli_fetch_array($sql)){
					$item = $item+1;
                                        $codigo= "'".$mostrar['codigo_tm']."'".",'".trim($mostrar['movimiento'])."'";
					echo '<tr>
