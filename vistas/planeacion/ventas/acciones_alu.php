...
switch ($_GET['sw']){
    ...
    case 27:
        ...
        if($idlam==''){
        mysqli_query($con,"INSERT INTO `cotizacion_item` (`modulo`,`id_parametro_vidrio`,`hoja_vidrio`,`total_mob`,`id_cot`, `codigo`, `descripcion_principal`,`trazabilidad`, `descripcion_segunda`, `ancho`, `alto`, `cantidad`, `laminas`, `perforacion`, `boquete`, `pelicula`, `carton`, `linea_cot`, `id_cot_principal`, `ubicacion`, `observacion`, `item`, `instalaccion`, `valor_item`, `descuento`, `iva`, `fecha_registro`, `usuario`, `estado`, `por_vid`, `por_alu`, `por_acc`) "
                . "VALUES ('$modulo' ,'$idparvid' ,'$desparvid','$mob' ,'$cot', '$cod', '$des', '$traz', '$des0', '$ancho', '$alto', '$cant', '$lam', '$per', '$boq', '$pelicula', '$carton', 'Vidrio', '$item', '$ubc', '$obse', '$itemv', '$inst', '$valor', '$desc', '19', '$fecha', '$usuario', 'En proceso','$despvid','$despalu','$despacc');");
        $id = mysqli_insert_id($con);
        $error = mysqli_error($con);
        
        }else{
            mysqli_query($con,"update cotizacion_item set  modulo='$modulo' ,id_parametro_vidrio='$idparvid' ,hoja_vidrio='$desparvid' ,codigo='$cod' ,total_mob='$mob' ,trazabilidad='$traz',descripcion_principal='$des',descripcion_segunda='$des0',laminas='$lam',cantidad='$cant',valor_item='$valor',por_vid='$despvid',por_alu='$despalu',por_acc='$despacc' where id_cot_item='$idlam' ");
            $id = $idlam;
            $error = mysqli_error($con);
        }
    ...
    case 36:
       ...
       echo '<ul>';
        ...
        case 39:
            ...
            $sistema = $r[0];
            $result = mysqli_query($con, "SELECT * FROM receta_kits a, receta_kits_sistemas b where a.idkit=b.idkit and b.sistema='$sistema' and a.modulo='Cierres' ");
            $c = 0;
           echo '<ul>';
        while($f = mysqli_fetch_array($result)){
             $des = "'".$f[1]."'";
        ...
        $id = $_GET['id'];
            ...
            $r = mysqli_fetch_array($result2);
            $sistema = $r[0];
            $result = mysqli_query($con, "SELECT * FROM receta_kits a, receta_kits_sistemas b where a.idkit=b.idkit and b.sistema='$sistema' and a.idkit in ($id) ");
            $c = 0;
        $no = '';
        while($f = mysqli_fetch_array($result)){
            $no =  $no.', '.$f[1];
    ...
    echo $no;
        ...
        break;
         case 40:
            ...
            $result = mysqli_query($con, "SELECT * FROM receta_kits a, receta_kits_sistemas b where a.idkit=b.idkit and b.sistema='$sistema' and a.modulo='Rodajas' ");
            $c = 0;
            echo '<ul>';
            while($f = mysqli_fetch_array($result)){
                $des = "'".$f[1]."'";
                echo '<li><input type="radio" id="riel'.$f[0].'" name="el" onclick="seleccionarro('.$f[0].','.$des.')">  '.$f[1].'</li>';
           }
           $des = "'N/A'";
        ...
        case 40.11:
            ...
            $result = mysqli_query($con, "SELECT * FROM receta_kits a, receta_kits_sistemas b where a.idkit=b.idkit and b.sistema='$sistema' and a.idkit='$id' ");
            $c = 0;
            $f = mysqli_fetch_array($result);
            if($f[1]){
            echo $f[1];
        ...
        break;
            ...
            $result = mysqli_query($con, "SELECT * FROM receta_kits a, receta_kits_sistemas b where a.idkit=b.idkit and b.sistema='$sistema' and a.modulo='Brazos' ");
            $c = 0;
              echo '<ul>';
            while($f = mysqli_fetch_array($result)){
                $des = "'".$f[1]."'";
                echo '<li><input type="radio" id="riel'.$f[0].'" name="el" onclick="seleccionarbr('.$f[0].','.$des.')">  '.$f[1].'</li>';
           }
           $des = "'N/A'";
        ...
        break;
            ...
            $result = mysqli_query($con, "SELECT * FROM receta_kits a, receta_kits_sistemas b where a.idkit=b.idkit and b.sistema='$sistema' and a.modulo='Bisagras' ");
            $c = 0;
               echo '<ul>';
            while($f = mysqli_fetch_array($result)){
                $des = "'".$f[1]."'";
                echo '<li><input type="radio" id="riel'.$f[0].'" name="el" onclick="seleccionarbi('.$f[0].','.$des.')">  '.$f[1].'</li>';
           }
           $des = "'N/A'";
...
//                }
            ...
            echo '<tr><td  style="text-align:center;background: #438EB9" colspan="12">Rodajas</td></tr>';
...
$p = explode(",",$cancie);
            ...
            . '<td style="text-align:right"><input type="hidden" id="total_insumo_desp3" value="'.$total_insumo_desp3.'" style="width:100%">$ '.number_format($total_insumo_desp3).'</td>';
             ...
             echo '<tr><td  style="text-align:center;background: #438EB9" colspan="12">Brazos</td></tr>';
            ...
            . '<td style="text-align:right"><input type="hidden" id="total_insumo_desp4" value="'.$total_insumo_desp4.'" style="width:100%">$ '.number_format($total_insumo_desp4).'</td>';
               ...
               echo '<tr><td  style="text-align:center;background: #438EB9" colspan="12">Bisagras</td></tr>';
        ...
        case 44:
            ...
            while($f = mysqli_fetch_array($result)){
                ...
                $result2 = mysqli_query($con, "select costo_calculado,area from productos_var where referencia = '".$f[2]."' ");
                $p = mysqli_fetch_array($result2);
                $precio = $p[0];
                $perimetro = $p["area"]/1000;
