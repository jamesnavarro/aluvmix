 ...
 $query = mysqli_query($con, "select * from cotizacion_item where id_cot_principal='$item' and estado='Guardado' ");
    ...
    while($rg = mysqli_fetch_array($query)){
        ...
        if($gen==0){
         ...
         $resultvi = mysqli_query($con, "select * from  producto_vidrios where id_pv='".$rg['id_parametro_vidrio']."' ");
               $f = mysqli_fetch_array($resultvi);
                $vidrio = $f[2];
                $vref1 = $f[3];
                $vope1 = $f[4];
                $vvar1 = $f[5];
                ...
                include '../productos_dos/formula_vidrios.php';
                
                $ancho_vidrio = $resultadov;
                $alto_vidrio  = $resultadov2;
...
$tound = $ggt / $can;
 ...
 mysqli_query($con, "update cotizacion_item set anchocfd='$ancfd',altocfs='$alcfs',anchocfi='$ancfi',altocfi='$alcfi',altorej='$rej',."
         . "+item='$itemv',ancho='$ancho', alto='$alto', cantidad='$can',perforacion='$per',boquete='$boq',pelicula='$pel',instalaccion='$ins',valor_item='$precio_venta_total', descuento='$descuento',por_esp='$por_esp',por_int='$por_int',por_vid='$por_vid',por_alu='$por_alu',por_acc='$por_acc', observacion='$obse',ubicacion='$ubc'  where id_cot_item='$item' ");
mysqli_error($con);
