<?php
	...
	$request = mysqli_query($con2,'SELECT COUNT(*) FROM cotizacion a, cont_terceros b WHERE a.id_tercero = b.id_ter '. $pre . $freg . $cot . $nom . $obr . $reg . $ana . $est . $pedi );
	if ($request) {
...
<?php
	...
	$request_ac = mysqli_query($con2,"SELECT * FROM cotizacion a, cont_terceros b
								WHERE a.id_tercero = b.id_ter " . $pre . $freg . $cot . $nom . $obr . $reg . $ana. $est. $pedi."
