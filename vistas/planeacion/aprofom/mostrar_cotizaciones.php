<?php
	include "../../../modelo/conexionv1.php";
		...
		if ($_POST['reg'] != '') {
			...
			$reg = '';
                ...
                if ($_POST['ped'] != '') {
			$pedi = ' AND a.pedido like "%' . $_POST['ped'] . '" ';
		} else {
			$pedi = '';
		}
			...
			$est = '';
                ...
                if ($_POST['freg'] != '' && $_POST['fregf'] != '') {
			$freg = ' AND a.ped_fechapedido >= "'.$_POST['freg'].'" AND a.ped_fechapedido <= "'.$_POST['fregf'].'" ';
		} else {
			$freg = '';
		}
                if ($_POST['pre'] != '') {
	...
	$request = mysqli_query($con2,'SELECT COUNT(*) FROM cotizacion a, cont_terceros b WHERE a.id_tercero = b.id_ter and a.pedido!="" '. $pre . $freg . $cot . $nom . $obr . $reg . $ana . $est . $pedi );
	if ($request) {
...
<?php
	}  echo 'Cantidad de registro: <b>'.$num_items.'</b> |  <a href="../vistas/planeacion/aprofom/reporte_pedido.php?cot='.$_POST['cot'].'&nom='.$_POST['nom'].'&obr='.$_POST['obr'].'&reg='.$_POST['reg'].'&ana='.$_POST['ana'].'&est='.$_POST['est'].'&freg='.$_POST['freg'].'&fregf='.$_POST['fregf'].'&pre='.$_POST['pre'].'&ped='.$_POST['ped'].'">Descargar de Excel </a>';
        echo '</td></tr>';
		...
		if ($request_ac) {
			...
			$v = '';
				...
				if ($row["fecha_guardado"] == '0000-00-00 00:00:00') {
					...
					$tiempo3 = '<font color="green">' . $row['ped_fechapedido'] . '</font><br>' . $tiempos3;
...
//				  }
                                        ...
                                        if($row['pedido']==''){
                                            ...
                                            $btnver = '<button type="button" onclick="ver_cotizacion('.$row['id_cot'].','.$linea.');"><img src="../images/view.png" /> </button>';
					...
					$estanu = '<a href="http://172.16.0.40/planeacion/vistas/?id=op&cot='.$row["id_cot"].'&cli='.$row["id_tercero"].'&ped='.$row["orden"].'&t=Empresarial&pedfom='.$row["pedido"].'" target="_blank"><button type="button" onclick="buscar_ord('.$row['id_cot'].')">Crear OP</button></a>';
				}
				...
				$table = $table . '<td width="15%">' . strtoupper($nombre) . '<br>' . $led . ' ' . $tiempo33 . '<br>' . strtoupper($row["obra"]) . '</td>';
			
				$table = $table . '<td width="10%">Reg :<b>' . $row["fecha_reg_c"] . '</b><br>Mod :' . $row["fecha_modificacion"] . '<br>Apro::'.$tiempo3.'</td>';
                                $table = $table . '<td class="hidden-phone"><b>' . ($row["ped_registrado"]) . '<br> <span style="font-size:8px">'.$row['ped_obs'].'</span></td>';
...
//				$table = $table . '<td width="10%" style="text-align:right;">$' . number_format($row["costo_total"]) . '<br>'.$sg.'</td>';
                                $table = $table . '<td class="hidden-phone">'.$estanu.'</td>';
