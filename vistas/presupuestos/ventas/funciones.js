...
$(function() {
    ...
    $("#add_mat").click(function () {
        ...
        $("#mat_linea").change(function () {
           mostrar_ajustes(1);           
        });
        $("#espa").change(function () {
           mostrar_ajustes(1);           
        });
        $("#mat_can").change(function () {
            calcular_venta();
        });
        $("#mat_des").change(function () {
            calcular_venta();
        });
        $("#mat_uti").change(function () {
            calcular_venta();
        });
        $("#mat_med2").change(function () {
            calcular_perfiles();
        });
        $("#mat_can2").change(function () {
            calcular_perfiles();
        });
        $("#mat_uti2").change(function () {
            calcular_perfiles();
        });
        $("#mat_des2").change(function () {
            calcular_perfiles();
        });
        $("#mat_desp").change(function () {
            calcular_venta();
        });
        $("#mat_uti").change(function () {
            calcular_venta();
...
function calcular_venta(){
    var can = $("#mat_can").val(); 
            ...
            var uti = $("#mat_uti").val();
            var cf = $("#mat_cf").val();
            var cft = (100 - parseFloat(cf) + parseFloat(uti))/100;
            ...
            $("#mat_valt").val(t);
            //$("#mat_td").val(tp);
            //$("#mat_ven").val(tpt);totalizar_perfiles()
             setTimeout(totalizar_accesorios,1000);
             
}
function calcular_servicios(){
    $("#saventa3").attr("disabled",false);
    var precio = $("#mat_precio").val();
    var can = $("#mat_can3").val();
    var par = (100-$("#mat_par").val())/100;
    var des = (100-$("#mat_des3").val())/100;
    var neto = ((precio * can)/par)*des;
    var und = neto / can;
    $("#mat_valt3").val(neto.toFixed(2));
    $("#mat_val3").val(und.toFixed(2));
}
...
function planilla2(){
    var linea = $("#mat_linea").val();
     var t_alu =0;
         var t_vid = 0;
         var t_acc = $("#mat_valt").val();
           var despalu = 0;
         var despacc = $("#mat_desp").val();
         var despace = 0;
         var totdesvid =0;
     ...
     window.open("../vistas/presupuestos/costos/planilla_costo.php?item="+item+"&utilidad="+utilidad+"&cot="+cot+"&gt="+gt+"&total_comp="+total_comp+"&totdesvid="+totdesvid+"&total_comp_desp="+total_comp_desp+"&desp="+desp+"&despalu="+despalu+"&despacc="+despacc+"&despace="+despace+"&t_alu="+t_alu+"&t_vid="+t_vid+"&t_acc="+t_acc+"&t_mob="+t_mob+"&t_ins="+t_ins+"&t_pol="+t_pol,"Planilla","width=1200,height=800");
}
function planilla3(){
  
     var t_alu =$("#mat_pt2").val();
     var t_vid = 0;
     var t_acc = 0;
     var despalu = $("#mat_desp2").val();
     var despacc = 0;
     var despace = 0;
     var totdesvid =0;
...
function update_ced(){
     var idc = $("#idc").val();
    window.open("../vistas/presupuestos/ventas/index.php?id="+idc,"terceros","width=300,height=300");
}
       ...
       var ase = $("#ase").val();
        var ana = $("#ana").val();
         ...
         var pag = $("#pag").val();
          ...
          if(idc===''){
              alert("Debes escoger el cliente");
              $("#doc").focus();
              return false;
          }
          if(exp===''){
...
function pre_cotizar(id){
    ...
    if(linea=='Vidrio'){
          window.open("../vistas/presupuestos/ventas/formulario.php","Cotizador","width=1400,height=800");
    }else{
          window.open("../vistas/presupuestos/ventas/aluminio.php","Cotizador","width=1400,height=800");
    }
...
function pre_cotizar_editar(id,linea){
    ...
    //console.log(idglobal);
    if(linea=='Vidrio'){
          window.open("../vistas/presupuestos/ventas/formulario.php","Cotizador"+id+"","width=1400,height=800", "_blank");
    }else{
          window.open("../vistas/presupuestos/ventas/aluminio.php","Cotizador"+id+"","width=1400,height=800", "_blank");
    }
...
function agregar_compuesto(){
    ...
    function calcular_item(n,item,gen){
        ...
        $.ajax({
                 post:'GET',
                 data:'item='+item+'&utilidad='+utilidad+'&gen='+gen+'&ubc='+ubc+'&por_esp='+por_esp+'&por_int='+por_int+'&obse='+obse+'&itemv='+itemv+'&cod='+cod+'&pel='+pel+'&ins='+ins+'&cant='+cant+'&ancho='+ancho+'&alto='+alto+'&per='+per+'&boq='+boq+'&desc='+desc+'&por_vid='+por_vid+'&por_alu='+por_alu+'&por_acc='+por_acc+'&por_ace='+por_ace+'&sw=38',
                 //url:'presupuestos/ventas/calcular_general.php',
...
function camiva(){
    ...
    $.ajax({
      ...
      function open_material(){
               window.open('../popup/productos_var/','Productos','width=900 , height=600');
      }
      function open_material2(){
               window.open('presupuestos/popup/cuentas/?linea=Aluminio','Productos','width=900 , height=600');  
      }
      function open_servicios(){
               window.open('presupuestos/popup/servicios/?linea=Instalacion','Productos','width=900 , height=600');  
      }
      function enviar_servicios(ref,desc,pre){
          $("#mat_cod3").val(ref);
          $("#mat_desc3").val(desc);
          $("#mat_precio").val(pre);
      }
     ...
     function add_producto(){
         ...
         var pre = $("#mat_val").val();
         var linea = $("#mat_linea").val();
         var desp = $("#mat_desp").val();
         var mat_td = $("#mat_td").val();
         var pt = $("#mat_pt").val();
         var gt = $("#mat_gt").val();
         var col = $("#mat_col").val();
         var item = $("#mat_items").val();
          var aca = 0;
          if(cod==''){
              alert("Debes de seleccionar el codigo del producto");
              $("#mat_cod").focus();
              return false;
          }
          if(can==''){
          ...
          console.log('linea ='+linea);
           if(linea=='' || linea==null){
              alert("Debes de seleccionar la linea");
              $("#mat_linea").focus();
              return false;
          }
         $.ajax({
                 post:'GET',
                 data:'idcot='+idcot+'&mat_td='+mat_td+'&item='+item+'&col='+col+'&pt='+pt+'&aca='+aca+'&gt='+gt+'&id='+id+'&linea='+linea+'&desp='+desp+'&pre='+pre+'&cod='+cod+'&can='+can+'&med='+med+'&des='+des+'&uti='+uti+'&obs='+obs+'&sw=39',
                 url:'presupuestos/ventas/acciones.php',
                 success : function(r){
                     var p = eval(r);
                     $("#mat_id").val(p[0]);
   ...
   function add_producto2(){
         var idcot = $("#idcot").val();
         var cod = $("#mat_cod2").val();
         var can = $("#mat_can2").val();
         var med = $("#mat_med2").val();
         var des = $("#mat_des2").val();
         var uti = $("#mat_uti2").val();
         var obs = $("#mat_obs2").val();
         var id = $("#mat_id2").val();
         var pre = $("#mat_val2").val();
         var linea = $("#mat_linea2").val();
         var desp = $("#mat_desp2").val();
         var mat_td = $("#mat_td2").val();
         var pt = $("#mat_pt2").val();
         var gt = $("#mat_gt2").val();
         var aca = $("#mat_aca").val();
          var col = $("#mat_col2").val();
          var item = $("#mat_items2").val();
           if(cod==''){
              alert("Debes de seleccionar el codigo del producto");
              $("#mat_cod2").focus();
              return false;
         ...
         $.ajax({
                 post:'GET',
                 data:'idcot='+idcot+'&item='+item+'&mat_td='+mat_td+'&col='+col+'&pt='+pt+'&aca='+aca+'&gt='+gt+'&id='+id+'&linea='+linea+'&desp='+desp+'&pre='+pre+'&cod='+cod+'&can='+can+'&med='+med+'&des='+des+'&uti='+uti+'&obs='+obs+'&sw=39',
                 url:'presupuestos/ventas/acciones.php',
                 success : function(r){
                     var p = eval(r);
                     $("#mat_id2").val(p[0]);
     ...
     function pasar_acc(cod,des,col,pre){
         $("#mat_cod").val(cod);
         $("#mat_desc").val(des);
         $("#mat_col").val(col);
         $("#mat_val").val(pre);
      ...
      function add_producto3(){
         var idcot = $("#idcot").val();
         var cod = $("#mat_cod3").val();
         var desc = $("#mat_desc3").val();
         var precio = $("#mat_precio").val();
         var pro = $("#mat_pro").val();
         var can = $("#mat_can3").val();
         var para = $("#mat_par").val();
         var des = $("#mat_des3").val();
         var uni = $("#mat_val3").val();
         var total = $("#mat_valt3").val();
         var obs = $("#mat_obs3").val();
         var id = $("#mat_id3").val();
         var item = $("#mat_items3").val();
         $.ajax({
                 post:'GET',
                 data:'idcot='+idcot+'&desc='+desc+'&item='+item+'&precio='+precio+'&pro='+pro+'&para='+para+'&uni='+uni+'&id='+id+'&total='+total+'&cod='+cod+'&can='+can+'&des='+des+'&obs='+obs+'&sw=43',
                 url:'presupuestos/ventas/acciones.php',
                 success : function(r){
                     console.log(r);
                     var p = eval(r);
                     ...
                     mostrar_servicios(idcot);
                     limpiarservicios();
                     mostrar_items(idcot);
                     
                      $("#saventa3").attr("disabled",true);
                 }
             });
     }
     ...
     function totalizar_accesorios(){
    
     var t_alu = 0;
     var despalu = 0;// aluminio mas desperdicio
     var t_vid = 0; //total costo vidrio
     var totdesvid = 0;  // vidrio mas desperdicio

     var t_acc = $("#mat_valt").val(); // total costo de accesorios

     var t_mob = 0; // total de mano de obratotdesvid
     var t_ins = 0;
     var t_pol = 0; // total de pelicula
     var gt = $("#mat_ven").val();
     var desp = $("#desperdicio").val();    // x no
     
     var despacc = $("#mat_desp").val();// x no
     var despace = 0;// x no
     var utilidad = $("#mat_uti").val();
     var total_espa = 0; // costo de espaciadores e interlayer
     var total_espa_desp = 0; // espaciadores  + desperdicio
     
     var d = ($("#mat_des").val()/100);
     var item = 0;
     ...
     // gt="+gt+"&total_comp="+total_comp+"&totdesvid="+totdesvid+"&total_comp_desp="+total_comp_desp+"&desp="+desp+"&despalu="+despalu+"&despacc="+despacc+"&despace="+despace+"&t_alu="+t_alu+"&t_vid="+t_vid+"&t_acc="+t_acc+"&t_mob="+t_mob+"&t_ins="+t_ins+"&t_pol="+t_pol
             $.ajax({
                    type:'GET',
                    data:'gt='+gt+'&item='+item+'&utilidad='+utilidad+'&total_espa='+total_espa+'&totdesvid='+totdesvid+'&total_espa_desp='+total_espa_desp+'&desp='+desp+'&despalu='+despalu+'&despacc='+despacc+'&despace='+despace+'&t_alu='+t_alu+'&t_vid='+t_vid+'&t_acc='+t_acc+'&t_mob='+t_mob+'&t_ins='+t_ins+'&t_pol='+t_pol,
                    url:'presupuestos/ventas/calculo_costo_alu.php',
                    success : function(t){
                          $("#mat_pt").val(t);
                           totaldes = t * d;
                           totalr = parseFloat(t) + parseFloat(totaldes);
                           $("#mat_gt").val(totalr);
                            $("#saventa").attr("disabled", false);
                         
                    }
                });
}
function totalizar_perfiles(){
    
     var t_alu = $("#mat_pt2").val();
     var despalu = $("#mat_desp2").val();// aluminio mas desperdicio mat_td2
     var t_vid = 0; //total costo vidrio
     var totdesvid = 0;  // vidrio mas desperdicio

     var t_acc = 0; // total costo de accesorios
     ...
     var utilidad = $("#mat_uti2").val();
     var total_espa = 0; // costo de espaciadores e interlayer
     var total_espa_desp = 0; // espaciadores  + desperdicio despalu
     
     var d = ($("#mat_des2").val()/100);
     var item = 0;
...
function calcular_perfiles(){
    ...
    var des = $("#mat_des2").val();
    var por = $("#mat_desp2").val();
    var uti = $("#mat_uti2").val();
             $.ajax({
                 post:'GET',
                 data:'cod='+cod+'&col='+col+'&med='+med+'&can='+can+'&uti='+uti+'&des='+des+'&por='+por+'&sw=41',
                 url:'presupuestos/ventas/acciones.php',
                 success : function(r){
                     var p = eval(r);
                     $("#mat_val2").val(p[0]);
                     ...
                     $("#mat_pt2").val(p[3]);
                     $("#mat_aca").val(p[4]);
                     setTimeout(totalizar_perfiles,1000);
                 }
             });
}
function ver_ventas(id){
    $("#modalproductos").modal('show');
    $("#saventa").attr("disabled",false);
    $.ajax({
                 post:'GET',
                 ...
                 success : function(r){
                     ...
                     $("#mat_col").val(p[16]);
                     $("#mat_desc").val(p[17]);
                     $("#mat_valt").val(p[9]*p[4]);
                     var desp = (100 - p[13]) /100;
                     var td = ((p[9]*p[4]) / desp);
                     $("#mat_td").val(td.toFixed(2));
                     $("#mat_items").val(p[20]);
                     $("#saventa2").attr('disabled',false);
                 }
             });
...
function info(id,ite){
 ...
 function salvar_com(){
       ...
       $.ajax({
                type:'GET',
                data:'sw=45&tex='+encodeURIComponent(tex)+'&id='+id,
                url:'presupuestos/ventas/acciones.php',
                success:function(d){
    ...
    function mostrar_desglose(){
        var idc = $("#idcot").val();
        window.open("presupuestos/ventas/desglose.php?idc="+idc,"Desglose","width=1400px , height=800px");
    }
    ...
    function upimg(){
        var idc= $("#idcot").val();
         window.open("../vistas/presupuestos/ventas/imagenes.php?cot="+idc,"Desglose","width=1400px , height=800px");
    }