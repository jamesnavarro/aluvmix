...
if(isset($_SESSION['k_username'])){
    ...
    $cp=$_GET['cp'];
    $page= $_GET['page'];
            $request = mysqli_query($con,"SELECT count(*) FROM centrocostos where cen_codigo like '%".$cod."%' and cen_nombre like '%".$des."%' and cen_nomrem like '%".$res."%' and cod_centropro like '%".$cp."%' ");
...
$request_ac = mysqli_query($con,"SELECT * FROM centrocostos where cen_codigo like '%".$cod."%' and cen_nombre like '%".$des."%' and cen_nomrem like '%".$res."%' and cod_centropro like '%".$cp."%'  " .$limit );
$total2=0;
 while($fila=mysqli_fetch_array($request_ac))
{  
    if ($fila['cen_activo']== '1') {
...
$cod = "'".$fila['cen_codigo']."'";
        echo '<tr>'
        . '<td>'.$fila['cen_codigo'].'</td>'
        . '<td>'.$fila['cen_nombre'].'</td>'
        . '<td>'.$fila['cen_nomrem'].'</td>'
        . '<td>'.$fila['cod_centropro'].'</td>'
        . '<td><img src="'.$estado.'"></td>'
        . '<td>'.$fila['cen_fechamod'].'</td>'
        . '<td>'.$fila['usuario'].'</td>'
