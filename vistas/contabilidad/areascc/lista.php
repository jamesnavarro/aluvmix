...
if(isset($_SESSION['k_username'])){
    ...
    $res=$_GET['res'];
   $page= $_GET['page'];
            $request = mysqli_query($con,"SELECT count(*) FROM areascc where are_codigo like '%".$cod."%' and are_nombre like '%".$des."%' and are_nomrem like '%".$res."%' ");
...
$request_ac = mysqli_query($con,"SELECT * FROM areascc where are_codigo like '%".$cod."%' and are_nombre like '%".$des."%' and are_nomrem like '%".$res."%'  " .$limit );
 $total2=0;
	while($fila=mysqli_fetch_array($request_ac))
	{  
 if ($fila['are_activo']== '1') {
...
$cod = "'".$fila['are_codigo']."'";
        ...
        . '<td>'.$fila['are_codigo'].'</td>'
        . '<td>'.$fila['are_nombre'].'</td>'
        . '<td>'.$fila['are_nomrem'].'</td>'
                 . '<td>'.$fila['are_fechamod'].'</td>'
                 . '<td>'.$fila['usuario'].'</td>'
