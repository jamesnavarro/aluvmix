...
if($request){
	...
	{    
		...
		if ($row["linea_cot"] == 'Aluminio') {
			...
			$fi3 = mysqli_fetch_array(mysqli_query($s3));
			$mult = $fi3["p"] / 100;
			} else {
            ...
            while ($fila=mysqli_fetch_array($compu)){
                ...
                $fix =mysqli_fetch_array(mysqli_query($sx));
                $multx= $fix["p"]/100; 
...
if($fv25['color_v']==''){$v25 = '';}else{$v25 = '+'.$fv25['color_v'];}
            ...
            $fila21 =mysqli_fetch_array(mysqli_query($sql21));
      if($row['pelicula']=='No Aplica'){
            ...
            $fia =mysqli_fetch_array(mysqli_query($pdlr));
