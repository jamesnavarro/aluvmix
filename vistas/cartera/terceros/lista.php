...
if(isset($_SESSION['k_username'])){
    ...
    $tpdoc=$_GET['tpd'];
    $page= $_GET['page'];
            $request = mysqli_query($con, "SELECT count(*) FROM cont_terceros where cod_ter like '%".$cod."%' and nom_ter like '%".$des."%' and estado_ter like '%".$est."%' and ciudad_ter like '%".$res."%' and municipio_ter like '%".$tpdoc."%'");
...
$request_ac = mysqli_query($con, "SELECT *  FROM cont_terceros where cod_ter like '%".$cod."%' and nom_ter like '%".$des."%' and estado_ter like '%".$est."%' and ciudad_ter like '%".$res."%' and municipio_ter like '%".$tpdoc."%'" .$limit );
$total2=0;
...
$enviar=$fila['id_ter'].",'".trim($fila['nom_ter'])."'";
       ...
       . '<td><img style="cursor:pointer" onclick="programar('.$enviar.')" src="../imagenes/call.png" width="20px">'
        .'<td><img style="cursor:pointer" onclick="editar_ter('.$cod.')" src="images/modificar.png" <a data-toggle="tab" href="#lin2"</a></td>'
        . '</td>';
  }
