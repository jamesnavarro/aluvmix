...
switch ($_GET['sw']) {
        case 1:
            ...
            if($f[0]==0){
                $ver=mysqli_query($con,"insert into cont_terceros (`cod_ter`,`digiver_ter`,`nom_ter`,`doc_ter`,`dir_ter`,`telfijo_ter`,`telmovil_ter`,`municipio_ter`,`ciudad_ter`,`pais_ter`,`fecnac_ter`,`correo_ter`,`cont_ter`,`pal`,`pvi`,`pac`,`estado_ter`,`especial`,`fuente`,`ica`,`iva`,`cree`,`vendedor`,`tipo_ter`) "
            ...
            break;
                case 2:
                 $ids=$_GET['id'];
                 $query = mysqli_query($con,"SELECT * FROM cont_terceros where cod_ter='$ids' ");
                 $fila = mysqli_fetch_array($query);
                 $p = array();
                  $p[0]=$fila['cod_ter'];
                 $p[1]=$fila['tipo_ter'];
                 $p[2]=$fila['doc_ter'];
                 $p[3]=$fila['digiver_ter'];
                 $p[4]=$fila['nom_ter'];
                 $p[5]=$fila['dir_ter'];
        ...
        case 8:
            ...
            $cel=$_GET['cel'];
            $ema=$_GET['ema'];
            $ver = mysqli_query($con,"update cont_terceros set nom_ter='$nom', cod_ter='$doc', telfijo_ter='$cel',  correo_ter='$ema', modificado='".$_SESSION['k_username']."',modificadofecha='".date("Y-m-d H:i:s")."'  where id_ter='$id'");
            if($ver){
                echo '<font color="green"><b>Se modifico con exito '.$nom.'</b></font>';
            }else{
                echo '<font color="red"><b>Error al editar'. mysqli_error($con).'</b></font>';
            }
            break;
