...
if(isset($_GET['page'])){
     ...
     if($_GET['cod']!=''){
                    $cod = ' and a.codigo='.$_GET['cod'];
            }else{
                    $cod = '';
            } 
                 if($_GET['des']!=''){
                    $des = ' and a.descripcion like "%'.$_GET['des'].'%" ';
            }else{
                    $des = '';
            }
                 if($_GET['ref']!=''){
                    $ref = ' and a.referencia like "%'.$_GET['ref'].'%" ';
            }else{
                    $ref = '';
            }
                 if($_GET['col']!=''){
                    $col = ' and a.color like "%'.$_GET['col'].'%" ';
            }else{
                    $col = '';
            }
                 if($_GET['med']!=''){
                    $med = ' and a.ancho='.$_GET['med'];
            }else{
                    $med = '';
            }
                 if($_GET['lin']!=''){
            ...
            $bod=$_GET['bod'];
$request=mysqli_query($con,"SELECT count(*) FROM conf_referencias a, referenciasalm b, relacion_ubicaciones c where b.cod_bod='$bod' and a.id_referencia=b.id_referencia and c.stock_ubi!=0 and a.id_referencia=c.id_referencia and a.stock_gen!=0 $cod $des $ref $col $med $lin $ubi ");
            if($request){
            ...
            $last_page = ceil($num_items/$rows_by_page);
                ...
                ?>
                        ...
                        $sql = mysqli_query($con,"SELECT * FROM conf_referencias a, referenciasalm b, relacion_ubicaciones c where b.cod_bod='$bod' and a.id_referencia=b.id_referencia and c.stock_ubi!=0 and a.id_referencia=c.id_referencia and a.stock_gen!=0  $cod $des $ref $col $med $lin $ubi ".$limit);
			$item = 0;
...
<td><a href="#PasarVariable" onclick="pasar('.$mostrar['codigo'].','.$nombre.')">'.$mostrar['descripcion'].'</a></td>'
                                                ...
                                                . '<td style="text-align:right">'.number_format($mostrar['costo_mt']).'</td>'
                                                ...
                                                . '<td><input type="text" id="cantidadx'.$mostrar['id_ru'].'" style="width:100%"></td>'
                                                . '<td><input type="text" disabled id="ubicacion'.$mostrar['id_ru'].'" style="width:100%" value="'.$mostrar['ubicacion'].'"></td>'
                                                . '<td><button onclick="adicionar('.$mostrar['id_ru'].')">+</button></td>'
                                               . '<input type="hidden"  id="idr'.$mostrar["id_ru"].'"  value="'.$mostrar['id_referencia'].'" />'
